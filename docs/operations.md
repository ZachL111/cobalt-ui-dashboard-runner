...
`cobalt-ui-dashboard-runner` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for dashboard scenarios with deny and allow fixtures, explainable decision traces, and no production deployment claims.
...
The score is deterministic. The current threshold is 154. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 5.
