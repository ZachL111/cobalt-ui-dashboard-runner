...
`cobalt-ui-dashboard-runner` explores frontend apps in Lua. The repository keeps the core rule set compact, then surrounds it with examples that show how the decisions move.

## Cobalt UI Dashboard Runner Notes
...
`degraded` is the first example I would inspect because it lands on the `review` path with a score of -14. The broader file also keeps `degraded` at -14 and `surge` at 216, which gives the model a useful low-to-high spread.
